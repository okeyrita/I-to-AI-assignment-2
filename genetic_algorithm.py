'''
Genetic algorithm by Margarita Peregudova.
...
# Get averega color of polygon 3x3, where (x,y) is a centre of polygon.
def get_average_color(image, x, y):
    pixel_color = []
    pixel_color.append(image.getpixel((abs(x-1), min(511, y+1))))
    pixel_color.append(image.getpixel((x, min(511, y+1))))
    pixel_color.append(image.getpixel((min(511, x+1), min(511, y+1))))
    pixel_color.append(image.getpixel((abs(x-1), y)))
    pixel_color.append(image.getpixel((x, y)))
    pixel_color.append(image.getpixel((min(511, x+1), y)))
    pixel_color.append(image.getpixel((abs(x-1), abs(y-1))))
    pixel_color.append(image.getpixel((x, abs(y-1))))
    pixel_color.append(image.getpixel((min(511, x+1), abs(y-1))))
    ...
    red = math.floor(sum_red/9)
    green = math.floor(sum_green/9)
    blue = math.floor(sum_blue/9)
...
def main():
    ...
    for circle in circles:
        pixel_color = get_average_color(image, circle.x, circle.y)
        ...
        while fitness_coef < QUALITY:
            child_circle = copy.deepcopy(circle)
    ...
    # Save result image.
    canvas.save(OUTPUT_DIRECTORY+OUTPUT_NAME)
